...
The machine-readable contracts are published as a [versioned JSON Schema catalogue](indicators/data/schemas/manifest.json). Consumers should pin active [schema package 1.1.0](indicators/data/schemas/1.1.0/manifest.json) or immutable package 1.0.0 and follow the [schema guide](docs/SCHEMA-GUIDE.md); a successful structural validation does not imply source verification, semantic equivalence, computability, comparability, privacy clearance, domain review or custodian approval.
...
Unified discovery is governed by [object taxonomy `PHAOS-OBJECT-TAXONOMY-001`](docs/UNIFIED-REGISTRY-TAXONOMY.md) and [identity policy `PHAOS-IDENTITY-001`](docs/IDENTITY-LIFECYCLE-POLICY.md). The Registry is now the common interface for a 4,743-canonical-object overview, 3,751 indicators, 1,046 HMIS knowledge objects and 560 individually searchable report-column occurrences, while representing the 54 shared registry/HMIS indicators only once canonically. Its filter-aware summaries show active filters, object-class counts and counting boundaries so an indicator subset cannot be mistaken for the complete knowledge layer. The generated, versioned [consolidated read model](indicators/data/registry/manifest.json) preserves source-specific metadata and supports reciprocal navigation among related canonical objects, occurrences, source evidence and matching graph nodes. This is a metadata and identity layer, not a dataset of observed health values.

The [observation-ready model `PHAOS-OBSERVATION-001`](docs/OBSERVATION-READY-MODEL.md) defines a separate privacy- and provenance-aware structure for future aggregate results and a distinct contract for targets and reference values. Its current manifest deliberately contains zero observations, zero reference values and zero public distributions.

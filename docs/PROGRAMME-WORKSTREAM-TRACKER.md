...
3. **UNIFY-01–UNIFY-10 — Unified registry — In progress.** UNIFY-01 object taxonomy through UNIFY-09 observation-ready modelling are complete. Next automate the integrated validation and release gate under UNIFY-10.
...
5. **UNIFY-05 — Consolidated read model — Complete.** The generated, versioned read model publishes 4,743 deduplicated canonical records, 4,797 source representations and 560 report occurrences. It keeps indicator and HMIS metadata in separate namespaces for the 54 shared identities, derives governed programme/system facets at build time, and is now the sole browser source for indicator and HMIS record arrays. The manifest records inputs, byte size and SHA-256; a validator checks identity, membership, type, occurrence and facet invariants. Public bulk-download controls remain disabled.
6. **UNIFY-06 — Report-schema occurrence layer — Complete.** All 560 version-specific report columns are first-class searchable records: 525 measures and 35 dimensions. The Registry provides card/table views plus class, module/category and linkage-status filters; occurrence detail exposes source wording, code, ordinal, schema, version, applicability, reconciliation and review metadata. The 508 accepted links resolve to canonical identities, while 17 candidates remain held outside the 765 canonical HMIS data-element count. The consolidated read model is version 1.1.0 with a validated occurrence discovery envelope.
7. **UNIFY-07 — Relationship navigation — Complete.** The generated read model 1.2.0 contains 536 governed relationship assertions represented as 1,072 reciprocal entries, 508 reverse report-occurrence links, 4,797 source-evidence paths and 1,564 graph-node links, with zero unresolved targets. Indicator and HMIS detail views expose connected canonical objects, report occurrences and sources; accepted occurrences return to their canonical HMIS object; graph objects return to Registry metadata. Candidate report fields retain an explicit no-canonical-target boundary.
8. **UNIFY-08 — Result summaries — Complete.** Indicator, HMIS-object and report-occurrence views now show filter-aware totals, active filters, class breakdowns and leading categories. The HMIS summary explicitly states that 54 derived indicators are one class within the 1,046-object HMIS knowledge layer; occurrence summaries separately preserve the 4,743-canonical-object and 765-data-element counting boundaries.
9. **UNIFY-09 — Observation-ready model — Complete.** Adopted `PHAOS-OBSERVATION-001` and schema package 1.1.0. Future aggregate observations have independent identity, exact grain, canonical subject linkage, period, provenance, revision, privacy and suppression metadata; targets and thresholds use a separate reference-value contract. The publication contains zero values and zero public distributions.

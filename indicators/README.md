...
UNIFY-01 through UNIFY-05 establish the governed unified discovery foundation. [`PHAOS-OBJECT-TAXONOMY-001`](../docs/UNIFIED-REGISTRY-TAXONOMY.md) distinguishes indicators, data elements, outputs, validation rules and report occurrences; [`PHAOS-IDENTITY-001`](../docs/IDENTITY-LIFECYCLE-POLICY.md) reconciles 4,743 canonical objects and 4,797 source representations while deduplicating the 54 shared registry/HMIS indicators. The Registry now provides one overview with indicator, HMIS, report-schema and graph views. Its indicator and HMIS views are derived from [`data/registry/unified-read-model.json`](data/registry/unified-read-model.json), whose versioned manifest and validation script make the browser projection reproducible without overwriting source-specific metadata.

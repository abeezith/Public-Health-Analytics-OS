...
UNIFY-01 through UNIFY-07 establish the governed unified discovery foundation. [`PHAOS-OBJECT-TAXONOMY-001`](../docs/UNIFIED-REGISTRY-TAXONOMY.md) distinguishes indicators, data elements, outputs, validation rules and report occurrences; [`PHAOS-IDENTITY-001`](../docs/IDENTITY-LIFECYCLE-POLICY.md) reconciles 4,743 canonical objects and 4,797 source representations while deduplicating the 54 shared registry/HMIS indicators. The Registry provides indicator, HMIS and report-occurrence card/table explorers plus its graph view. All are derived from [`data/registry/unified-read-model.json`](data/registry/unified-read-model.json); version 1.2.0 adds reciprocal navigation among related canonical objects, occurrences, source evidence and matching graph nodes without inflating canonical counts or overwriting source metadata.

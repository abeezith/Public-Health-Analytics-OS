...
const fields = ['search','domain','type','source','pillar','sort'];
...
$('clear').addEventListener('click', () => { $('search').value=''; $('domain').selectedIndex=0; $('type').selectedIndex=0; $('source').selectedIndex=0; $('pillar').selectedIndex=0; filter(); });
...
function filter() {
  state.visible = 24;
  const q = $('search').value.trim().toLowerCase(), domain=$('domain').value, type=$('type').value, source=$('source').value, pillar=$('pillar').value, sort=$('sort').value;
  state.filtered = state.indicators.filter(x => {
    ...
    return (!q || text.includes(q)) && (domain==='All domains'||x.domain===domain) && (type==='All types'||x.type===type) && (source==='All sources'||x.source===source) && (pillar==='All WHO pillars'||(x.whoPillars||[]).includes(pillar));
  ...
  $('clear').hidden = !(q || domain!=='All domains' || type!=='All types' || source!=='All sources' || pillar!=='All WHO pillars');
  render();
}
function render() {
...
function card(x){ return '<article class="indicator-card"><div class="card-top"><span>'+esc(x.id)+'</span>'+(x.code?'<span>'+esc(x.code)+'</span>':'')+'</div><div class="card-tags"><span class="card-domain">'+esc(x.domain)+'</span><span class="pillar-tag">'+esc(x.whoPillarPrimary||'Unclassified')+'</span></div><h3>'+esc(x.name)+'</h3><p>'+esc(x.definition)+'</p><dl><div><dt>Collection</dt><dd>'+esc(x.collection)+'</dd></div><div><dt>WHO pillars</dt><dd>'+esc((x.whoPillars||[]).join('; ')||'Not classified')+'</dd></div></dl><button data-open="'+esc(x.id)+'">View full metadata <span>→</span></button></article>'; }

...
fetch('./data/indicators.json?v=measure-1.0.0', { cache: 'no-store' }).then(r => r.json()).then(data => {
...
function filter() {
  ...
  state.filtered = state.indicators.filter(x => {
    ...
    const text=[x.id,x.name,x.domain,x.subdomain,x.code,x.definition,x.displayDefinition,x.source,x.org,x.uses,x.country,x.indiaProgram,x.indiaReportingSystem,x.programmeComponent,x.reportingLevel,x.reportingUnit,x.responsibleCadre,x.measureType,x.scaleDisplay,x.normalizedFormula,x.denominatorPopulation].join(' ').toLowerCase();

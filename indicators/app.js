...
const fields = ['search','geography','program','component','level','domain','type','source','pillar','sort'];

fetch('./data/indicators.json?v=hmis-1.3.0', { cache: 'no-store' }).then(r => r.json()).then(data => {
...
$('clear').addEventListener('click', () => { $('search').value=''; ['geography','program','component','level','domain','type','source','pillar'].forEach(id => $(id).selectedIndex=0); filter(); });
...
function filter() {
  state.visible = 24;
  const q = $('search').value.trim().toLowerCase(), geography=$('geography').value, program=$('program').value, component=$('component').value, level=$('level').value, domain=$('domain').value, type=$('type').value, source=$('source').value, pillar=$('pillar').value, sort=$('sort').value;
  state.filtered = state.indicators.filter(x => {
    const geo=x.country || 'Global / multi-country';
    const text=[x.id,x.name,x.domain,x.subdomain,x.code,x.definition,x.source,x.org,x.uses,x.country,x.indiaProgram,x.indiaReportingSystem,x.programmeComponent,x.reportingLevel,x.reportingUnit,x.responsibleCadre].join(' ').toLowerCase();
    return (!q || text.includes(q)) && (geography==='All geographies'||geo===geography) && (program==='All India programmes'||x.indiaProgram===program) && (component==='All programme components'||x.programmeComponent===component) && (level==='All lowest reporting levels'||x.lowestReportingLevel===level) && (domain==='All domains'||x.domain===domain) && (type==='All types'||x.type===type) && (source==='All sources'||x.source===source) && (pillar==='All WHO pillars'||(x.whoPillars||[]).includes(pillar));
  ...
  $('clear').hidden = !(q || geography!=='All geographies' || program!=='All India programmes' || component!=='All programme components' || level!=='All lowest reporting levels' || domain!=='All domains' || type!=='All types' || source!=='All sources' || pillar!=='All WHO pillars');
  render();
}
function render() {
...
function openModal(id){
  ...
  const indiaMeta=x.country?meta('Country / scope',x.country)+meta('India programme',x.indiaProgram)+meta('Programme component',x.programmeComponent)+meta('Object type',x.objectType||x.recordType)+meta('Reporting system',x.indiaReportingSystem)+meta('Lowest reporting level',x.lowestReportingLevel)+meta('Full reporting levels',x.reportingLevel)+meta('Reporting unit',x.reportingUnit)+meta('Responsible cadre',x.responsibleCadre)+meta('Administrative level',x.administrativeLevel)+meta('Facility type',x.facilityType)+meta('Record type',x.recordType)+meta('Source document',x.sourceDocument)+meta('Source section',x.sourceSection)+meta('Source item code',x.sourceItemCode)+meta('Source version',x.sourceVersion)+meta('Aggregation rule',x.aggregationRule,true)+meta('Zero-denominator rule',x.zeroDenominatorRule,true)+meta('Linked HMIS data elements',x.relatedElementIds,true)+meta('Data lineage',x.lineage,true)+meta('Currentness',x.currentness,true)+meta('Source location',x.sourcePage,true):'';
